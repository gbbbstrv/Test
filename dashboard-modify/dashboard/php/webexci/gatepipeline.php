...
where spr.PULLREQUESTID=sprlog.PULLREQUESTID and spr.STATUS=para.PARAMKEY and para.PARAMGROUP='SCMPullRequestStatus'
...
where ecs.status=param.paramkey and param.paramgroup='ECJobStepStatus' and ecs.STEPSEQUENCEID=seq.STEPINDEX and ecs.STEPID=log.STEPID order by STEPID,LOGTIME desc 
) step 
...
//            $ecstepjob->url = "http://cadada.com";
        ...
        if ($row["PRLOGTIME"] != null) {
            if (in_array($row["PRLOGTIME"], $prlogindex, true)) {
                ...
                $log->url = "https://bitbucket-eng-chn-sjc1.cisco.com/bitbucket/projects/" . $row["STASHPROJECT"] . "/repos/" . $row["REPOSITORY"] . "/pull_requests/" . $row["PULLREQUESTID"] . "/overview";
                array_push($logs, $log);
            }
        }
        if ($row["MERGETIMESTAMP"] != null) {
        ...
        if ($row["STEPID"] != null) {
            if (in_array($row["STEPLOGTIME"], $steplogindex, true)) {
                ...
                $log->log = $row["STEPNAME"] . ":" . $row["STEPSTATUS"] . "--" . $row["STEPLOG"] . " ";
                $log->url = "https://cctg-ec2.cisco/commander/link/jobStepDetails/jobSteps/";
                array_push($logs, $log);
            }
        }
