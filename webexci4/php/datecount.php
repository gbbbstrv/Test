...
if($starttime=="" and $endtime==""){
    ...
    for($i = time(); $i >= time()-30*24*3600; $i -= 86400)
    {
        $days[]=date("Y-m-d",$i);
    }
}
...
$str="select DATE_FORMAT(time,'%Y-%m-%d') as 'date_format',count(*) as 'count'  from commitinfo where project='cctg'  ";
$str2="select DATE_FORMAT(time,'%Y-%m-%d') as 'date_format',count(*) as 'count'  from commitinfo where project='cctg-fork'  ";
if($starttime1!=''){
...
$str2=$str2."and  type='UPDATE'  and branch ='refs/heads/master' group by date_format order by date_format desc limit 30";
